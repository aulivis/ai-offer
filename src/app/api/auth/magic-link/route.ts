...
import { validateAndNormalizeEmail } from '@/lib/validation/email';
import {
...
export const POST = withErrorHandling(async (request: NextRequest) => {
  ...
  // Validate and normalize remember_me - accept boolean or string 'true'
  const rememberMeRaw = body?.remember_me;
  const rememberMe =
    rememberMeRaw === true ||
    rememberMeRaw === 'true' ||
    (typeof rememberMeRaw === 'string' && rememberMeRaw.toLowerCase() === 'true');

  // Validate and normalize email
  const normalizedEmail = validateAndNormalizeEmail(emailInput);
  if (!normalizedEmail) {
    return createErrorResponse('Invalid email address format', HttpStatus.BAD_REQUEST);
  }

  // Check rate limit before processing
  try {
    const rateLimitKey = hashMagicLinkEmailKey(normalizedEmail);
