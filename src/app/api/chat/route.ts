/**
 * Chat API Route - Fallback/Redirect Route
 * 
 * This route exists as a fallback for the useChat hook which may default to /api/chat.
 * It forwards all requests to /api/chatbot to maintain compatibility.
 * 
 * POST /api/chat
...
export async function GET() {
  return Response.json({ 
    status: 'ok', 
    message: 'Chat API is available. Use /api/chatbot for chatbot functionality.',
    redirect: '/api/chatbot'
  });
}
...
export async function POST(req: NextRequest) {
  ...
  log.info('Chat API route called, forwarding to /api/chatbot', {
    method: req.method,
    url: req.url,
    forwarded: true,
  });
  ...
  try {
    ...
    // Note: We don't try HTTP forwarding as fallback because the request body
    // can only be read once, and it's already been consumed by the direct handler call attempt
    return Response.json(
      {
